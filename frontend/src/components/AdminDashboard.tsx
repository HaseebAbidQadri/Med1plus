...
const photoSlots = [
  ...
  },
  {
    ...
    desc: 'Dr. Haseeb Ahmed\'s safe consultancy counter, assisting local customers 24/7.'
  },
  {
...
export default function AdminDashboard({ 
  ...
  const [newMedCategory, setNewMedCategory] = useState('Pain Relief');
  const [newMedPrice, setNewMedPrice] = useState('');
  const [newMedPurchasePrice, setNewMedPurchasePrice] = useState('');
  const [newMedStock, setNewMedStock] = useState('');
  const [newMedExpiry, setNewMedExpiry] = useState('2026-12-15');
  ...
  const handleAddMedicineSubmit = async (e: React.FormEvent) => {
    ...
    const purchasePriceNum = parseFloat(newMedPurchasePrice) || priceNum * 0.7; // default margin
    const stockNum = parseInt(newMedStock);

    const medPayload = {
      name: newMedName,
      generic: newMedGeneric || 'Generic Formula',
    ...
    setNewMedName('');
    setNewMedGeneric('');
    setNewMedCategory('Pain Relief');
    setNewMedPrice('');
    setNewMedPurchasePrice('');
    setNewMedStock('');
                          ...
                          <div key={ord.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100/50 gap-2">
                            ...
                            <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                              ...
                              {ord.status === 'Pending' ? (
                                ...
                                <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold flex items-center space-x-1 ${
                                  ord.status === 'Confirmed' ? 'bg-sky-50 text-[#0d5cb5] border border-sky-100' :
                ...
                <div className="flex items-center border border-slate-200 bg-white rounded-xl p-1 shadow-sm">
                  <button 
                    ...
                    className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                      activeMedicinesSubTab === 'all-list' ? 'bg-emerald-50 text-[#10b981]' : 'text-slate-500 hover:text-slate-800'
                    }`}
                  >
                    All Medicines
                  </button>
                  <button 
                    ...
                    className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                      activeMedicinesSubTab === 'add-form' ? 'bg-emerald-50 text-[#10b981]' : 'text-slate-500 hover:text-slate-800'
                    }`}
                  >
                    + Add Medicine
                  </button>
              ...
              {activeMedicinesSubTab === 'all-list' ? (
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto">
                      <div className="flex items-center space-x-1.5 shrink-0">
                        ...
                        >
                          ...
                          <option value="Vitamins & Supp.">Vitamins & Supp.</option>
                          ...
                          {filteredMedicines.map((item) => (
                            <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                              <td className="py-3 px-6 flex items-center space-x-3.5">
                                <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100 text-slate-500 flex-shrink-0">
                                  <Pill className="w-4 h-4" />
                                </div>
                                <div className="text-left">
                                  <span className="text-xs font-bold text-slate-800 block leading-tight">{item.name}</span>
                                  <span className="text-[9px] text-[indigo-500] font-semibold mt-0.5 tracking-wide block">{item.generic}</span>
                                </div>
                              </td>
                              ...
                              <td className="py-3 px-4 text-center">
                                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold inline-block text-center tracking-wider max-w-[110px] ${
                                  item.status === 'In Stock' ? 'bg-emerald-50 text-[#10b981] border border-emerald-100' :
                ...
                <div className="bg-white p-6 sm:p-8 rounded-[32px] border border-slate-100 shadow-sm max-w-3xl">
                  ...
                  <form onSubmit={handleAddMedicineSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-left">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Generic / Salt Formula</label>
                      <input
                        type="text"
                        value={newMedGeneric}
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Purchase Import Price (PKR)</label>
                      <input
                        type="number"
                        value={newMedPurchasePrice}
                        onChange={(e) => setNewMedPurchasePrice(e.target.value)}
                        placeholder="e.g. 100"
                        className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#10b981] text-xs font-semibold outline-none transition-all"
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Batch Expiration Date</label>
                      <input
                        type="date"
                        value={newMedExpiry}
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700">
                      {medicinesList.map((item) => (
                        <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-right font-bold text-slate-500">PKR {item.purchasePrice}</td>
                          <td className="py-3 px-4 text-right font-extrabold text-[#10b981]">PKR {item.price}</td>
                          <td className="py-3 px-4 text-center font-extrabold">{item.stock}</td>
                          <td className="py-3 px-4 text-center font-bold text-slate-500">{item.expiryDate}</td>
                          <td className="py-3 px-4 text-center">
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold block mx-auto text-center w-24 ${
                              item.status === 'In Stock' ? 'bg-emerald-50 text-[#10b981]' :
                      ...
                      {ordersList.map((ord) => (
                        <tr key={ord.id} className="hover:bg-slate-50/50 transition-all">
                          ...
                          <td className="py-4 px-4 text-center">
                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider inline-block text-center ${
                              ord.status === 'Pending' ? 'bg-rose-50 text-rose-600' :
                      ...
                      <div>
                        ...
                        <div className="relative rounded-2xl overflow-hidden aspect-[4/3] bg-slate-100 border border-slate-200 mb-4 flex items-center justify-center group">
                          <img 
                            ...
                            referrerPolicy="no-referrer"
                          />
                          <div className={`absolute top-2 left-2 px-2 py-0.5 rounded-md border text-[8px] font-black uppercase text-white ${
                            customImage ? 'bg-[#10b981]/90 border-emerald-400' : 'bg-slate-900/60 border-slate-700'
                          }`}>
